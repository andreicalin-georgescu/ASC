...
class CoffeeFactory(Thread):
    """CoffeeFactory a.k.a the producer"""
...
def main():
    parser = argparse.ArgumentParser(description = "Producer - consumer coffee machine")
    parser.add_argument("-p", type = int, required = True, dest = "NR_PROD", help = "the number of producers")
    parser.add_argument("-c", type = int, required = True, dest = "NR_CONS", help = "the number of consumers")
    parser.add_argument("-b", type = int, required = True, dest = "BUFF_SIZE", help = "the size of the buffer")
    ...
    buff = Distributor(args.BUFF_SIZE)

    producers = [None] * args.NR_PROD
    consumers = [None] * args.NR_CONS

    for i in range(args.NR_PROD):
        producers[i] = CoffeeFactory(i, buff, sizes, coffees)
        producers[i].start()

    for i in range(args.NR_CONS):
